...
namespace uiuiui
{
    public partial class EmployeeManagement : UserControl
    {
        ...
        {
            conn.Open();
            string query = "insert into EMPLOYE (MSNV,HOTEN,CMND,GIOITINH,DT,LUONG,CALV, CHUCVU)values('" + tbmanv.Text + "','" + tbtennv.Text + "','" + tbcmnd.Text + "','" + comboBoxsex.Text + "','" + tbsdt.Text + "','" + tbluong.Text + "','" + comboBoxcalv.Text + "','" + comboBoxvitri.Text + "')";
            SqlDataAdapter sda = new SqlDataAdapter(query, conn);
            sda.SelectCommand.ExecuteNonQuery();
            conn.Close();
            load_data();
        ...
        {
            string query = "delete from EMPLOYE where MSNV='" + tbmanv.Text + "'";
            conn.Open();
            SqlDataAdapter sda = new SqlDataAdapter(query, conn);
            sda.SelectCommand.ExecuteNonQuery();
            conn.Close();
